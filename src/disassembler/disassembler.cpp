...
Disassembler::~Disassembler()
{
    delete[] this->romBuffer;
}

bool Disassembler::loadROM(char const* romPath)
{
    ifstream file(romPath, ios::binary | ios::ate);

    if(!file.is_open())
    {
        cerr << "Failed to load ROM" << romPath << endl;
        return 0;
    }

    streampos size = file.tellg();
    this->size = size;
    this->romBuffer = new unsigned char[size];
    file.seekg(0, ios::beg);
    file.read((char*)this->romBuffer, size);
    file.close();
...
void Disassembler::disassemble()
{
    ...
    {
        this->pc += this->disassembleOP();
    }
}

int Disassembler::disassembleOP()
{
    unsigned char* code = &this->romBuffer[this->pc];
    int opbytes = 1;    
    printf ("%04x ", this->pc);
    switch (*code)
	{
